...
@property (nullable, readonly) NSString *deviceId;
...
- (NSDictionary<OHQDeviceInfoKey,id> *)deviceInfo {
    ...
    if (deviceId) {
        ret[OHQDeviceInfoDeviceIdKey] = deviceId;
    }
...
- (instancetype)init {
    ...
    if (self) {
        ...
        [self ohq_convertManagerState:self.central.state toDeviceManagerState:&_state];
        
        _scanStartBlock = nil;
        _scanObserverBlock = nil;
        _scanCompletionBlock = nil;
        _discoveredDevices = [@{} mutableCopy];
        _sessionInfoDictionary = [@{} mutableCopy];
        
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ohq_observeNotification:) name:UIApplicationDidEnterBackgroundNotification object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ohq_observeNotification:) name:UIApplicationWillEnterForegroundNotification object:nil];
    }
    return self;
}
...
- (void)cancelSessionWithDevice:(NSUUID *)identifier complete:(void(^)(BOOL isCancel))complete{
   ...
   if (sessionInfo.device.peripheral.state == OHQConnectionStateConnected) {
       complete(NO);
   }else{
...
//    self.connentTimer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(connentPeripheral:) userInfo:@{@"peripheral":peripheral} repeats:YES];
//    [self.connentTimer fire];
}

- (void)connentPeripheral:(NSTimer *)timer {
  //连接外设
