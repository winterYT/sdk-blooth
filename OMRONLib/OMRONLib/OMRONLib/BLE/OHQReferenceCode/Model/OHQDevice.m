...
static NSString * const DeviceIdCharacteristicUUIDString = @"2A25";
...
static CBUUID * _deviceIdCharacteristicUUID = nil;
...
EventAttachedDataKey const EventAttachedDataParamKey = @"param";
...
@interface BeaconIdentifierReadingState : OHQState
@end
...
+ (void)initialize {
    if (self == [OHQDevice class]) {
        ...
        dispatch_once(&onceToken, ^{
            ...
            _deviceIdCharacteristicUUID = [CBUUID UUIDWithString:DeviceIdCharacteristicUUIDString];
...
- (instancetype)initWithPeripheral:(CBPeripheral *)peripheral queue:(dispatch_queue_t)queue {
    ...
    if (self) {
        ...
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
//            [DIParamReader sharedParamReader].queue = queue;
        });
        
        [self addState:[InactiveState state]];
        [self addState:[ActiveState state]];
        [self addState:[BeaconIdentifierReadingState state] toParentStateClass:[ActiveState class]];
...
- (BOOL)shouldHandleEvent:(NSUInteger)event withObject:(id)object {
    ...
    switch (event) {
        case StateMachineEventStartTransfer: {
            ...
            if ([device.options[OHQSessionOptionAllowAccessToBeaconIdentifierKey] boolValue]) {
                [device transitionToStateClass:[BeaconIdentifierReadingState class]];
            }
            else {
...
///---------------------------------------------------------------------------------------
#pragma mark - Beacon Identifier Reading State
///---------------------------------------------------------------------------------------

@implementation BeaconIdentifierReadingState

- (void)didEnter {
//    OHQDevice *device = (OHQDevice *)self.stateMachine;
    
//    [[DIParamReader sharedParamReader] readParams:@[DIParamProximityUUIDKey, DIParamMajorKey, DIParamMinorKey] fromPeripheral:device.peripheral withCompletion:^(CBPeripheral * _Nullable peripheral, NSDictionary<DIParamKey,id> * _Nullable paramValues, NSError * _Nullable error) {
//        NSMutableDictionary *obj = [@{} mutableCopy];
//        obj[EventAttachedDataParamKey] = paramValues;
//        obj[EventAttachedDataErrorKey] = error;
        [self.stateMachine updateWithEvent:StateMachineEventDidReadParam object:nil];
//    }];
}
...
- (BOOL)shouldHandleEvent:(NSUInteger)event withObject:(id)object {
    ...
    switch (event) {
        case StateMachineEventDidReadParam: {
//            NSDictionary<DIParamKey,id> *param = object[EventAttachedDataParamKey];
            NSError *error = object[EventAttachedDataErrorKey];
            handled = YES;
            
            if (error) {
                OHQFuncLogE(@"error:%@", error);
//               if ([error.domain isEqualToString:DIErrorDomain] && error.code == DIErrorPeripheralNotSupported) {
//                    [device transitionToStateClass:[ServicesDiscoveringState class]];
//                    break;
//                }
//                else {
...
//            data[OHQBeaconIdentifierDataProximityUUIDKey] = param[DIParamProximityUUIDKey];
//            data[OHQBeaconIdentifierDataMajorKey] = param[DIParamMajorKey];
//            data[OHQBeaconIdentifierDataMinorKey] = param[DIParamMinorKey];
            if (data.count) {
                device.dataObserverBlock(OHQDataTypeBeaconIdentifier, [data copy]);
            }
            [device transitionToStateClass:[ServicesDiscoveringState class]];
            break;
        }
...
- (void)didEnter {
    ...
    discoverCharacteristics(deviceInformationService, @[_deviceIdCharacteristicUUID]);
...
- (void)didEnter {
    ...
    CBCharacteristic *deviceIdStringCharacteristic = [deviceInformationService characteristicWithUUID:_deviceIdCharacteristicUUID];
    readCharacteristic(deviceIdStringCharacteristic);
        ...
        case StateMachineEventDidUpdateValueForCharacteristic: {
            ...
            if ([self.attributesOfInterest containsObject:characteristic]) {
                ...
                if ([characteristic.UUID isEqual:_modelNumberStringCharacteristicUUID]) {
                    ...
                    NSString *modelName = [NSString stringWithUTF8String:characteristic.value.bytes];
                ...
                if ([characteristic.UUID isEqual:_deviceIdCharacteristicUUID]) {
                    if(characteristic.value.bytes){
                   NSString *deviceId = [NSString stringWithUTF8String:characteristic.value.bytes];
                   device.dataObserverBlock(OHQDataTypeDeviceId, deviceId);
                    }
                }
...
- (void)ohq_device:(OHQDevice *)device didUpdateValueForDatabaseChangeIncrementCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if(appDatabaseChangeIncrementValue==0)
    {
        NSDictionary *userData = device.options[OHQSessionOptionUserDataKey];
        __block NSMutableArray<OHQUserDataKey> *missingUserDataKeys = [@[] mutableCopy];
        [device.supportedUserDataKeys enumerateObjectsUsingBlock:^(OHQUserDataKey  _Nonnull key, NSUInteger idx, BOOL * _Nonnull stop) {
