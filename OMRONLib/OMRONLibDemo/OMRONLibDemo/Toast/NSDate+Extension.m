...
#define TIME_ZONE_LOCAL [NSTimeZone timeZoneWithAbbreviation:@"localTime"]

@implementation NSDate (Extension)
- (NSTimeInterval)timeIntervalGMTSince1970
{
    //设置源日期时区
    ...
    return destinationDateNow.timeIntervalSince1970;
}
...
+ (NSString *)stringWithDate:(NSDate *)date format:(NSString *)format isUTC:(BOOL)isUTC{
	return [date stringWithFormat:format isUTC:isUTC];
}

- (NSString *)stringWithFormat:(NSString *)format isUTC:(BOOL)isUTC{
	NSDateFormatter *outputFormatter = [[NSDateFormatter alloc] init];
	[outputFormatter setDateFormat:format];
	if (isUTC) {
		outputFormatter.timeZone = TIME_ZONE_UTC;
	}
...
+ (NSDate *)dateWithString:(NSString *)string format:(NSString *)format {
    ...
    inputFormatter.timeZone = TIME_ZONE_UTC;
...
+ (NSDate *)dateWithString:(NSString *)string format:(NSString *)format isUTC:(BOOL)isUTC {
	NSDateFormatter *inputFormatter = [[NSDateFormatter alloc] init];
	[inputFormatter setDateFormat:format];
	if (isUTC) {
		inputFormatter.timeZone = TIME_ZONE_UTC;
	} else {
        inputFormatter.timeZone = [NSTimeZone timeZoneWithAbbreviation:@"localTime"];
    }
	NSDate *date = [inputFormatter dateFromString:string];
...
{
    //设置源日期时区
    NSTimeZone* sourceTimeZone = TIME_ZONE_UTC;//或GMT
    //设置转换后的目标日期时区
    NSTimeZone* destinationTimeZone = [NSTimeZone localTimeZone];
    //得到源日期与世界标准时间的偏移量
